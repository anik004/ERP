...
#			write(logfile,'POST:P:parameter set to default value')
		else:
			print 'POST:F:parameter not set to default value'
#			write(logfile,'POST:F:parameter not set to default value')
	else:
...
try:
    if argv[1] == "--u":
		...
		value = argv[5]
		location = argv[6]
		space_check(argv[1], argv[2], argv[3], argv[4],argv[5],argv[6],argv[7])

except Exception as e:
	if "No such file or directory" in str(e):
		print "No such file"
#		write(logfile,'POST:F:No such file')
		write('reflogfile.log','POST:F:No such file')
...
#		write(logfile,'POST:F:Please enter App for Application Server or Db for Database Server')
		write('reflogfile.log','POST:F:Please enter App for Application Server or Db for Database Server')
