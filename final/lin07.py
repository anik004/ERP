...
try:
	...
	#print schema
        ...
        command = 'sudo su - ' + user + ' -c "echo \'exit;\' | sqlplus -s  system/' + argv[5] + '" | grep -i "ERROR:" | grep -v "#"'
	stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
        if stdout.readlines():
            print "PRE:F: The System user ( system ) is not able to login to the server (HOSTNAME - " + hostname + ")"
        else:
            print "PRE:P: The System user ( system ) is able to login to the server (HOSTNAME - " + hostname + ")"
