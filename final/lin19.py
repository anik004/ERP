...
def export(filename, hostname, username, password, client_name, stepname, seqno, appsid,refresh_id, path):
    ...
    command = "sudo chmod 777 " + path.strip() + '/' + filename
    command = Popen(command,shell=True,stdout=subprocess.PIPE)
    out, err = command.communicate()

    file_open = open(path.strip() + '/' + filename, 'wt')
...
select * from """ + filename
    ...
    command = "sudo chmod 777 " + '/home/' + appuser
    print command
    stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)

    print path.strip() +  '/' + filename
    ...
    sftp.put (path.strip() + '/' + filename, '/home/' + appuser +  '/' + filename)
	...
	stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
        final = stdout.channel.recv_exit_status()
        if final == 0:
            print 'PRE:P:The backup for the table ' + filename + ' has been exported'
#	    write(refresh_id,'PRE:P:The backup for the table ' + filename + ' has been exported')
        else:
            print 'PRE:F:The backup for the table ' + filename + ' has been failed'
#	    write(refresh_id,'PRE:F:The backup for the table ' + filename + ' has been failed')
	    exit()
    else:
        print  'PRE:F:The permission change has been failed for the file ' + filename
#	write(refresh_id,'PRE:F:The permission change has been failed for the file ' + filename)
	exit()
...
#command = 'python ' + path + '/win14.py ' + argv[5].upper()
print command
command = Popen(command,shell=True,stdout=subprocess.PIPE)
out, err = command.communicate()
