...
def space_check(hostname, username, password, profilepath, value, location, logfile):
    ...
    if "False" in str(out):
        ...
        if 'rdisp/wp_no_btc' not in out:
            ...
            write(location.strip('\\') + '\\reflogfile.log',out)
            print out
            #command='c:\python27\python ' + locationation.strip('\\') + '\wmiexec.py ' + username.strip() + ':' + password.strip() + '@' + hostname + ' "powershell.exe;\"\\"(Get-Content ' + profilepath +').Replace(\'' + parameter + '\',\'rdisp/wp_no_btc = ' + value + '\') | set-content ' + profilepath + '"\\"\"'
            #print command
            #command=subprocess.Popen(command,shell=True,stdout=subprocess.PIPE)
            #out, err = command.communicate()
            #print out
