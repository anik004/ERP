...
def imp(filename, hostname, username, password, appsid, client_name, seqno, stepname, location, drive, logfile):
    #print username
#    client = SSHClient()
...
select * from """ + filename.strip()
    ...
    if status != 0:
	    ...
	    write(logfile,'PRE:F:File has not been created')
	...
	for each in data_1:
			...
			if final_status == '0000' or final_status == '0004':
        			...
        			write(logfile,stepname + ':P:The backup for the table ' + filename + ' is successful')
				...
				print stepname + ':F:The backup for the table ' + filename + ' has failed with error code:' + final_status
 				write(logfile,stepname + ':F:The backup for the table ' + filename + ' has failed with error code:' + final_status)
 			"""
    else:
        print stepname + ':F:The backup for the table ' + filename + ' has failed'
        write(logfile,stepname + ':F:The backup for the table ' + filename + ' has failed')
        exit()
...
try:
  ...
  for each in table_name:
    seqno=seqno + 1
    imp(each, argv[1], argv[2], argv[3], argv[4],argv[5],str(seqno),argv[6], argv[7], argv[8].strip('\\'), logfile)
