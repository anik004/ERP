...
#    print "Usage: python sapstopapp.py <Target application Host> <Target application Login User Name> <Target application User Password> <Target Application SID> <Instance ID> <Instance Host>"
 #   else:
        ...
        command = 'sudo su - ' + user_sap + ' -c \'cdpro; ls | grep -i ' + host + ' | grep -v "\." | grep -v "ASC" | cut -d"_" -f2\''
	print command
	...
	print command
        ...
        if status[0].strip() == "running":
            print "SAPSTART:P: The SAP service is already started on the target server (HOSTNAME - " + hostname + ")"
...
#		print command
                ...
                if stdout.channel.recv_exit_status() == 0:
                    print "SAPSTART:I: The processes related to the SAP has been stopped in the target application server (HOSTNAME - " + hostname + ")"
	    ...
	    print command
            ...
            if status == 0:
                print "SAPSTART:P: The SAP service has been started on the target server (HOSTNAME - " + hostname + ")"
                log = "POST:P: The SAP service has been started on the target server (HOSTNAME - " + hostname + ")"
                write (logfile, log)
            else:
                print "SAPSTART:F: The SAP server has not been successfully started on the target server (HOSTNAME - " + hostname + ")"
