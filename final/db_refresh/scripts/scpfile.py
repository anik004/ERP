	...
	try:
		...
		if dest == "source":
			...
			print command
				...
				status = stdout.channel.recv_exit_status()
			        ...
			        command = 'sudo su - ' + user + ' -c "echo @/oracle/' + database_sid.upper() + '/sapreorg/sql.sql | sqlplus system/Welcome2" | grep -i "ERROR"'
			        stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
			        if stdout.readlines():
		...
		if path_array is not None:
			for folder in path_array:
				command = "sudo su - " + user + " -c \'chmod -R 777 " + folder + "\'"
				print command
				stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
...
#	print target_arr
	...
	#	print "each"
        #       	print each
               	t = threading.Thread(target=scpfile, args=(sourcehostname, sourceusername, sourcepassword, sourcedatabase_sid.upper(), targethostname, user1, target_dbuser_password, targetdatabase_sid.upper(), each.strip()))
		#print t
       		t.start()
               	threads.append(t)
	for t in threads:
