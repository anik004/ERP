...
#	print status
	if status[0].strip() == "not":
	    ...
	    exit()
            ...
            command = 'sudo su - ' + user_sap + ' -c "stopsap r3 '  + instance + '"'
#	    print command
            stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
...
#        print command
	...
	#print status
	if "Error" in out[4]:
	    print 'SAPSTOP:F: Instance Id - ' + instance +  'passed does not exist on the target server (HOSTNAME - ' + host + ')'
	    log = 'PRE:F:Instance Id - ' + instance +  'passed does not exist on the target server (HOSTNAME - ' + host + ')'
	    write (logfile, log)
	    exit()
        if status == 0:
