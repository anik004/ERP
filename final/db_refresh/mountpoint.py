...
#	command = "df -m | awk '{s+=$4} END {print s}'"
	command = "df -h | grep -i sapdata* |  awk '{s+=$4}END {print s}'"
	print command
        stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
        free_space = int(stdout.readline())
        print 'free: ' +  str(free_space)
        total_space = free_space + target_server
        print 'total: ' + str(total_space)
