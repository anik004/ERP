...
def check(hostname,username,passwd,database_sid,user,dest,logfile):
	try:
		...
		if status != 0:
		        print "SCPFILE:F: Provided input for the " + dest + " Database SID ( " + database_sid + " ) in " + hostname + " host is incorrect"
			write(logfile,"POST:F: Provided input for the " + dest + " Database SID ( " + database_sid + " ) in " + hostname + " host is incorrect")
        		exit()
		...
		if dest == "source":
			...
			print command
				...
				status = stdout.channel.recv_exit_status()
			        ...
			        command = 'sudo su - ora' + database_sid + ' -c "echo \'alter database backup controlfile to trace as \'\\\'/oracle/' + database_sid.upper() + '/sapreorg/control_script_' + database_sid.upper() + '.sql\\\'\';\' > /oracle/' + database_sid.upper() + '/sapreorg/sql.sql;chmod 777 /oracle/' + database_sid.upper() + '/sapreorg/sql.sql"'
				print command
        			stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
			        status = stdout.channel.recv_exit_status()

			        command = 'sudo su - ora' + database_sid + ' -c "echo @/oracle/' + database_sid.upper() + '/sapreorg/sql.sql | sqlplus / as sysdba" | grep -i "ERROR"'
				print command
			        stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
			        if stdout.readlines():
		...
		if path_array is not None:
			for folder in path_array:
				command = "sudo su - ora" + database_sid + " -c \'chmod -R 777 " + folder + "\'"
				print command
				stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
...
try:
	...
	target_dbuser_password = argv[9]
	user1 = "ora" + targetdatabase_sid.lower()
	target = "target"
	path_array = []
	logfile = argv[10]

	target_arr = check(targethostname,targetusername,targetpassword,targetdatabase_sid,targetusername,target,logfile)
	print target_arr
	source_arr = check(sourcehostname,sourceusername,sourcepassword,sourcedatabase_sid,sourceusername,source,logfile)
	print source_arr
	...
	for folder in target_arr:
		command = "sudo su - " + user1 + " -c \'chmod 777 " + folder + "\'"
		print command
		stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
                status = stdout.channel.recv_exit_status()

	def scpfile(sourcehostname,sourceusername,sourcepassword,sourcedatabase_sid,targethostname,user1,target_dbuser_password,targetdatabase_sid,each):
               #	print sourceusername 
		command = "expect scp.exp " + sourcehostname + " " + sourceusername + " " + sourcepassword + " " + sourcedatabase_sid.upper() + " " + targethostname + " " + user1 + " " + target_dbuser_password + " " + targetdatabase_sid.upper() + " " + each
                print command
	...
	#	print "each"
        #       	print each
               	t = threading.Thread(target=scpfile, args=(sourcehostname, sourceusername, sourcepassword, sourcedatabase_sid.upper(), targethostname, targetusername, target_dbuser_password, targetdatabase_sid.upper(), each.strip()))
		#print t
       		t.start()
               	threads.append(t)
	for t in threads:
