	...
	print output
        if stdout.channel.recv_exit_status() == 0:
            if output=="ARCHIVELOG":
                print "PRE:W:Archive is enabled in the target database server ( Hostname - " + hostname + " )"
            else:
                print "PRE:P:Archive is disabled in target database server ( Hostname - " + hostname + " )"
        channel.close()
        client.close()
except Exception as e:
    if str(e) == "[Errno -2] Name or service not known":
        print "PRE:F:GERR_0901:Hostname unknown"
    elif str(e) == "list index out of range":
        print "PRE:F:GERR_0902:Argument/s missing for the script"
    elif str(e) == "Authentication failed.":
        print "PRE:F:GERR_0903:Authentication failed."
    elif str(e) == "[Errno 110] Connection timed out":
        print "PRE:F:GERR_0904:Host Unreachable"
