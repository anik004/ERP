...
try:
	if argv[1] == '--ani':
		...
		# ---------------------- get schema name ----------------------------
		write(location + '\\reflogfile.log' ,'win26: This command will execute the SQL query')
        ...
        if schema:
	# ----------------------- get table names ---------------------------
				write(location + '\\reflogfile.log' ,'win26: this command is used to get the tables name')
...
#                               command = 'c:\\python27\\python.exe ' + location.strip('\\') + '\\wmiexec.py ' + username.strip() + ':' + password.strip() + '@' + hostname.strip() + ' "sqlcmd -E -S ' + domain_name.strip() + '\\' + sid.upper() + ' -Q \\"use ' + sid.upper() + '; create index ' + table_name + '_' + field_name + ' on ' + schema + '.' + table_name + '(' + field_name + ')"'
					file_sql.write ('go')
					file_sql.close()
					write(location + '\\reflogfile.log' ,'win26 : This command is used to copy files files to the share folder')
					...
					out, err = command.communicate()
					write(location + '\\reflogfile.log' ,out)
					write(location + '\\reflogfile.log' ,'win26: this command is used execute SQL query ')
					...
					if status == 0 or status == 4:
						...
						write (location + '\\' + logfile,"POST:P:The indexes has been deleted successfully for the tables")
					else:
...
# ---------------------------- Exceptions ------------------------------------
	...
	if str(e) == "[Errno -2] Name or service not known":
		...
		write(location + '\\reflogfile.log' ,'POST:F:GERR_0205: Please check the hostname that you have provide')
