...
try:
	...
	profile_path = argv[11].rstrip('/')
		...
		command = 'sudo su - ' + t_sid.lower() + 'adm -c \'cd ' + profile_path + ' ;ls | grep -i "' + t_sid.upper() + '_DVE" | grep -v "\."\''
		print command
	        stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
        	profilefile = stdout.readlines()[0].strip()
	        print profilefile
        	command = 'sudo su - ' + t_sid.lower() + 'adm -c \'cd ' + profile_path + ' ;cat ' + profilefile + ' | grep -iw "DIR_TRANS" | grep -v "#"\''
	        print command
        	stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
        ...
        #    command = 'scp ' + s_path + "/" + name + "/" + initial + tr_part + "." + s_sid + " " + t_username.lower() + '@' + t_hostname + ':' + t_path + "/" + name
                ...
                localpath = s_path + "/" + name + "/" + initial + tr_part + "." + extension
                print localpath
                filepath =  t_path + "/" + name + "/" + initial + tr_part + "." + extension
                print filepath
		...
		write(logfile,"POST:P: The File " + initial + tr_part + "." + s_sid + " has been transfered successfully.")
                command = 'sudo chown ' + t_sid.lower() + 'adm:sapsys ' + t_path + '/' + name + '/' + initial + tr_part + '.' + extension + '; sudo chmod 777 ' + t_path + '/' + name + '/' + initial + tr_part + '.' + extension 
		print command
                client = SSHClient()
                client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                client.connect(t_hostname,username = t_username, password = t_password)
...
# schema passwd - $4
        ...
        command = 'sudo su - ' + t_sid.lower() + "adm" + ' -c "tp addtobuffer ' + tr_id + " " + t_sid  + ' Client=000 pf=' + t_path + '/bin/TP_' + domain + '.PFL "| grep -i \'tp finished with return code:\' | tail -n 1 | cut -d ":" -f2'
        print command
        stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
	...
	print status
        if status == "0" or status == "4":
            command = 'sudo su - ' + t_sid.lower() + 'adm -c "tp modbuffer ' + tr_id + " " + t_sid + ' mode=u+12 Client=000 pf=' + t_path + '/bin/TP_' + domain + '.PFL" |grep -i \'tp finished with return code:\' | tail -n 1 |cut -d ":" -f2'
            print command
            stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
            status = stdout.readline().rstrip().strip()
	    print status
            ...
            if status == "0" or status == "4":
                command = 'sudo su - ' + t_sid.lower() + 'adm -c "tp import ' + tr_id + " " +  t_sid + ' Client=000 pf=' + t_path + '/bin/TP_' + domain + '.PFL U123689"|grep -i \'tp finished with return code:\' |tail -n 1 | cut -d ":" -f2'
                print command
                stdin, stdout, stderr = client.exec_command(command, timeout=1000, get_pty=True)
                status = stdout.readline().rstrip().strip()
		print status
