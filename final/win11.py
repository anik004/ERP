...
# ------------------- Copy File -------------------------------------------------
    write(drive.strip('\\') + '\\reflogfile.log','win11: This command is used to copy files to sharefolder')
...
#    print status
    ...
    if "1" not in out:
        ...
        #write(drive.strip('\\') + logfile,'POST:F:The database script has not been created')
	# -------------------------------------- IMPORT -----------------------------
 ...
 #   print command
    ...
    if status == 0 or status == 4:
        ...
        for each in data_1:
            if 'R3trans finished' in each:
                ...
                if final_status == '0000' or final_status == '0004':
                    ...
                    #write(drive.strip('\\') + logfile,"POST:P:The import for the table ' + filename + ' is successful")
                else:
                    print 'POST:F:The import for the table ' + filename + ' is failed'
                    #write(drive.strip('\\') + logfile,'POST:F:The import for the table ' + filename + ' is failed')
                write(drive.strip('\\') + '\\reflogfile.log','win11: this command is used to delete the share folder')
        ...
        #write(drive.strip('\\') + logfile,'POST:F:The restoration for the table ' + filename + ' has failed')
try:
  ...
  for each in table_name:
    ...
    imp(each, argv[1], argv[2], argv[3], argv[4],argv[5],str(seqno),argv[6], argv[7], argv[8].strip('\\'))
        ...
        if str(e) == "[Errno -2] Name or service not known":
            ...
            write(drive.strip('\\') + '\\reflogfile.log','PRE:F:GERR_0202:Argument/s missing for the script')
            #write(drive.strip('\\') + logfile,'PRE:F:GERR_0202:Argument/s missing for the script')
