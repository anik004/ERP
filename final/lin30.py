...
#            print "PRE:F: The database user ( " + argv[5] + " ) is not able to login to the server (HOSTNAME - " + hostname + ")"
	    command = "sudo chmod 777 " + loc + '/' + refid + '_bdls.txt'
	    print command
	    command=subprocess.Popen(command,shell=True,stdout=subprocess.PIPE)
	    out, err = command.communicate()
	    
	    f = open(loc + '/' + refid + '_bdls.txt', "w+")
		...
		if "TABNAME" not in output[i].strip() and "-" not in output[i].strip() and output[i].strip() and "FIELDNAME" not in output[i].strip():
				...
				if "/" not in table_name:
					command = 'sudo su - ' + user + ' -c "echo \'create bitmap index ' + table_name + '_' + field_name + ' on ' + schema + '.' + table_name + '(' + field_name + ');\' | sqlplus / as sysdba"'
					print command
					#f.write(command + '\n')
					...
					if status != 0:
						print 'PRE:F:bitmap index creation failed'
						exit()
