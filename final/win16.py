...
def space_check(hostname, username, password, profilepath,logfile, location):
    ...
    if "False" in str(out):
        ...
        fo.write("3")
        fo.close()
        write(location.strip('\\') + '\\reflogfile.log','win16.py : This command is used to get the content and find the value of rdisp/wp_no_btc')
        command='c:\\python27\\python ' + location.strip('\\') + '\\wmiexec.py ' + username.strip() + ':' + password.strip() + '@' + hostname + ' "powershell.exe;\"\\"get-content ' + profilepath.strip() + ' | findstr rdisp/wp_no_btc\\"\" | findstr -V #"'
        print command
        write(location.strip('\\') + '\\reflogfile.log',command)
        command=subprocess.Popen(command,shell=True,stdout=subprocess.PIPE)
        out, err = command.communicate()
        ...
        if 'rdisp/wp_no_btc' not in out:
            ...
            print out
            write(location.strip('\\') + '\\reflogfile.log', out)
            write(location.strip('\\') + '\\reflogfile.log',"win16.py : This command is used to set the value of rdisp/wp_no_btc = 0")
            ...
            command = 'powershell.exe;"(Get-Content ' + location.strip('\\') + '\\' + profile + ') -Replace \'' + parameter + '\',\'rdisp/wp_no_btc = 0\' | set-content ' + location.strip('\\') + '\\' + profile + '"'
            print command
            write(location.strip('\\') + '\\reflogfile.log',command)
            command=subprocess.Popen(command,shell=True,stdout=subprocess.PIPE)
            out, err = command.communicate()
