...
#    	out, err = command.communicate()
	 command = 'c:\\python27\\python.exe ' + location.strip('\\') + '\wmiexec.py ' + t_username.strip() + ':' + t_password.strip() + '@' + t_hostname + ' "powershell.exe;test-path ' + t_profile_path + '"'
        command=subprocess.Popen(command,shell=True,stdout=subprocess.PIPE)
        out, err = command.communicate()
	if 'False' in out:
		print 'POST:F:The profilepath does not exist'
		write(ref_id,"POST:F:The profilepath does not exist")
		exit()
